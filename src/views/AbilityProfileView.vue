...
const failedStats = computed<Record<Exclude<SubmissionStatus, 'Accepted'>, number>>(() => ({
  'Wrong Answer': store.boundSubmissions.filter((item) => item.status === 'Wrong Answer').length,
  'Time Limit': store.boundSubmissions.filter((item) => item.status === 'Time Limit').length,
  'Runtime Error': store.boundSubmissions.filter((item) => item.status === 'Runtime Error').length,
}))
        ...
        <article class="panel panel-subtle">
          ...
          <div class="distribution-list">
            <div v-for="platform in platformDistribution" :key="platform.name">
              ...
              <div class="meter">
                <i :style="{ width: `${(platform.value / store.boundSubmissions.length) * 100}%` }" />
              </div>
