...
请求体包含用户设置、训练统计、弱项标签和近期提交样本。当前支持 OpenAI Compatible 和 DeepSeek 接口；DeepSeek 复用 OpenAI-compatible Chat Completions 协议。后端会请求 `${API Base URL}/chat/completions`，并要求返回可解析 JSON。
