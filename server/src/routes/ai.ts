...
function getProviderConfig(settings: AiProviderSettings) {
  ...
  const baseUrl = settings.aiBaseUrl?.trim() || (provider === 'deepseek' ? deepSeekDefaults.baseUrl : '')
...
router.post('/:userId/ai/advice', requireUser, async (req, res) => {
  ...
  try {
    ...
    if (!content) {
      return res.status(502).json({ error: `${getProviderLabel(config.provider)} 没有返回可用内容` })
    }
